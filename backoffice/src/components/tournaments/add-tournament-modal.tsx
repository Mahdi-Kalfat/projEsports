...
import { createTournament, type CreateTournamentState } from "@/app/(app)/tournaments/actions";
import { TournamentFormFields, type GameOption } from "./tournament-form-fields";
...
export function AddTournamentModal({ games }: { games: GameOption[] }) {
  const [open, setOpen] = useState(false);
  const [handledSuccess, setHandledSuccess] = useState(false);
  const [state, formAction] = useActionState(createTournament, initialState);

  // Close on success — adjusted during render (not in an effect: the modal fully
  // unmounts on close, which is also what resets the form's fields, so there's no
  // DOM work to do here, just reacting to the action's result exactly once per
  // submission via the handledSuccess guard).
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto p-4">
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="font-display text-base font-bold text-foreground">Add tournament</h2>
              <button
                type="button"
                onClick={() => setOpen(false)}
                aria-label="Close"
            ...
            <div className="mt-4">
              <TournamentFormFields games={games} />
            </div>
