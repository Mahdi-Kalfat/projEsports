...
export function TournamentCard({ tournament }: { tournament: TournamentCardData }) {
  const identity = getGameIdentity(tournament.gameName);
  const isLive = tournament.status === "LIVE";

  return (
    <motion.div whileHover={{ y: -6 }} transition={{ duration: 0.25, ease: "easeOut" }} className="relative h-full">
      {isLive && (
        <span
          aria-hidden
          className="absolute inset-y-0 left-0 z-10 w-1 rounded-l-2xl"
          style={{ background: identity.accent, boxShadow: `0 0 10px ${identity.accent}` }}
        />
      )}
      <Link
      ...
      >
        <div className="relative h-44 w-full overflow-hidden bg-gradient-to-br from-surface to-surface-raised">
          {tournament.backgroundImageUrl ? (
            ...
            <NoArtFallback gameName={tournament.gameName} />
          )}
          ...
          <span
            className={`absolute right-3 top-3 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium backdrop-blur-sm ${
          ...
          <div className="absolute left-3 top-3 flex flex-col items-start gap-1.5">
            {tournament.isJoined && (
              <span className="rounded-full bg-success px-2.5 py-1 text-xs font-semibold text-canvas shadow-[0_0_12px_rgba(40,255,139,0.5)]">
                Joined
              </span>
            )}
